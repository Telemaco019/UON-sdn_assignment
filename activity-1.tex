...
meet the requirements. To conclude, answer the questions proposed in the last task.
...
%a remote controller,).
A basic knowledge of the Python programming language is also assumed.
...
an additional example of how network with multiple controllers can be implemented
...
For creating the links, use the function \code{addLink} specifying the bandwidth
...
Mbit/s, while the delay specified with the parameter \code{delay} is expresed
...
to use remote controllers instead of local ones? (Hint: see reference \cite{ref-4})}
\hrulefill
